...
async function syncServerRows(rows: Array<Record<string, string>>, bindings: RuntimeBindings): Promise<{ created: number; updated: number }> {
  let created = 0, updated = 0;
  for (const row of rows) {
    const name = first(row, ["name"]); if (name === null) continue;
    const parts = name.split(/\s+/u), firstName = parts[0], lastName = parts.slice(1).join(" ");
    ...
    const nickname = first(row, ["nickname"]), upsell = optionalBlast(first(row, ["upsell_score", "upsell", "blast", "blast_percent", "blast_percentage", "blast_score"])), pitty = optionalInt(first(row, ["pitty", "pity"])), days = optionalInt(first(row, ["employment_days", "employment"])), capacity = optionalInt(first(row, ["max_guests", "capacity", "max_section_load"])), inTime = first(row, ["in_time", "clock_in", "clockin", "start_time"]);
    const start = new Date(Date.now() - (days ?? 0) * 86_400_000).toISOString().slice(0, 10);
    if (current === null) {
      await bindings.database.prepare("INSERT INTO employees(id,first_name,last_name,nickname,role,employment_start_date,active,upsell_score,pitty_score,employment_days,max_section_load,notes,created_at,updated_at) VALUES(?,?,?,?, 'SERVER',?,1,?,?,?,?,?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP)").bind(id, firstName, lastName, nickname, start, upsell, pitty, days, capacity, inTime).run();
      created += 1;
    } else {
      const changes: string[] = [], values: Array<string | number | null> = [];
      for (const [field,value] of [["nickname",nickname],["upsell_score",upsell],["pitty_score",pitty],["employment_days",days],["max_section_load",capacity],["notes",inTime]] as const) if (value !== null) { changes.push(`${field}=?`); values.push(value); }
      if (days !== null) { changes.push("employment_start_date=?"); values.push(start); }
      if (changes.length > 0) await bindings.database.prepare(`UPDATE employees SET ${changes.join(",")},updated_at=CURRENT_TIMESTAMP WHERE id=?`).bind(...values,id).run();
      updated += 1;
    }
  }
...
export async function routeImports(request: Request, url: URL, bindings: RuntimeBindings): Promise<Response | null> {
  ...
  if (url.pathname === "/imports/servers") {
    if (!headers.includes("name")) return apiError(request, 400, "CSV must include a 'name' column.");
    return jsonResponse(request, await syncServerRows(rows, bindings), { status: 201 });
  }
  if (url.pathname === "/imports/daily-roster") {
    ...
    const serverSync = await syncServerRows(rows, bindings);
    const store = storeParam === null ? null : Number(storeParam), entries = rows.map((row) => { const name = first(row,["name"]); if (name === null) return null; const inTime = first(row,["in_time","clock_in","clockin","start_time"]), nickname = first(row,["nickname"]), upsellScore = optionalBlast(first(row,["upsell_score","upsell","blast","blast_percent","blast_percentage","blast_score"])), pitty = optionalInt(first(row,["pitty","pity"])), employmentDays = optionalInt(first(row,["employment_days","employment"])), maxGuests = optionalInt(first(row,["max_guests","capacity","max_section_load"])); return { name, ...(inTime === null ? {} : { in_time: inTime }), ...(nickname === null ? {} : { nickname }), ...(upsellScore === null ? {} : { upsell_score: upsellScore }), ...(pitty === null ? {} : { pitty }), ...(employmentDays === null ? {} : { employment_days: employmentDays }), ...(maxGuests === null ? {} : { max_guests: maxGuests }) }; }).filter((entry) => entry !== null);
    ...
    return jsonResponse(request, { date, store_id: store, count: entries.length, employees_created: serverSync.created, employees_updated: serverSync.updated }, { status: 201 });
  }
